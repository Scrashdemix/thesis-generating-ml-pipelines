...
def generate(config: dict, root_dir_path: str) -> None:
    ...
    feature_pipeline_dir = generate_pipeline_file_structure(root_dir, 'feature_pipeline')
    feature_pipe = FeatureEngineering(config, root_dir, feature_pipeline_dir, last_datasets=data_pipe.get_next_datasets())
    # Training pipeline
    training_pipeline_dir = generate_pipeline_file_structure(root_dir, 'training_pipeline')
